...
def parse_bool_query(query):
	...
	print(ANDS)
	print("Interpreted query: %s" % ("(" + ") & (".join(" | ".join("^" + word[0] if word[1] == "-" else word[0] for word in AND) for AND in ANDS) + ")"))
	return ANDS
	...
	for OR in parsed_query:
		...
		for condition in OR:
			...
			if condition[1] == "+":
				...
				NOT = alldocs if condition[0] not in ifreq else alldocs.difference(set(elem[0] for elem in ifreq[condition[0]]))
