...
if __name__ == "__main__":
    for i, dirname in enumerate(os.listdir("../WIKIindexes/invwikialpha/")):
        # Yup, 678 folders, 26*26 + 2
        print "%s/677" % i, dirname
        final_dict = {}
        for j, filename in enumerate(os.listdir("../WIKIindexes/invwikialpha/" + dirname)):
            print "%s/64" % j, filename
            with open("../WIKIindexes/invwikialpha/" + dirname + "/" + filename, "r") as f:
        ...
        with open("../WIKIindexes/finalWiki/" + dirname + ".json", "w") as dump:
